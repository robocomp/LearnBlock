...
class Predictions(list):
    def __init__(self, iterable=list()):
        super(Predictions, self).__init__(iterable)

    def append(self, item):
        assert isinstance(item, SPrediction)
        super(Predictions, self).append(item)

    def extend(self, iterable):
        for item in iterable:
            assert isinstance(item, SPrediction)
        super(Predictions, self).extend(iterable)

    def insert(self, index, item):
        assert isinstance(item, SPrediction)
        super(Predictions, self).insert(index, item)

setattr(DetectionComponent, "Predictions", Predictions)
class ImgType(list):
    def __init__(self, iterable=list()):
...
setattr(DetectionComponent, "ImgType", ImgType)

import detectioncomponentI
...
class Implements:
    def __init__(self, ice_connector, default_handler):
        self.ice_connector = ice_connector
        self.detectioncomponent = self.create_adapter("DetectionComponent", detectioncomponentI.DetectionComponentI(default_handler))
